...
//tendencia  , triangulo y grafica
  ...
  //Dibujar cada arco dependiendo su nivel de alerta
    ...
    let rangoReverse=[]
    for(let i = 0 ; i<rango.length ;i++){ rangoReverse.push(rango[5-i]) }
  ...
  let sumatoria = 0;//sumatoria de todos los rangos antes del donde se encuentra el valor actual
    for (let i = 0; i<datoIndicador[1];i++){sumatoria+=rangoReverse[i];};
  let posicionValorActuar= 200- (((num-datoIndicador[0])/rangoReverse[datoIndicador[1]] )+ sumatoria)/total*200//indica hasta donde llega el arco , segun su valor actual
  console.log(nombre,200-((((num-datoIndicador[0])/rangoReverse[datoIndicador[1]] )+ sumatoria)/total) *200)
